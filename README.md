...
<img src="ironsight2.png" alt="IRONSIGHT Dashboard" width="100%">

# 🎯 IRONSIGHT

**Real-time OSINT command center for monitoring active conflicts.**

A single dashboard aggregating open-source intelligence from news, Telegram, military tracking, financial markets, satellite thermal data, and live air-threat feeds.
...
|---------|------|----------|
| Yahoo Finance | Stocks, indices, commodities, oil futures | Yahoo |
| CoinGecko | Cryptocurrency prices (BTC, ETH, SOL, BNB) | CoinGecko |
| Polymarket | Prediction-market odds | Polymarket (Gamma API) |
| NASA FIRMS | Fire / thermal-anomaly satellite data | NASA |
| adsb.lol | Military aircraft ADS-B tracking | Community ADS-B network |
| Tzeva Adom | Israeli missile/rocket/drone alerts | Community mirror of Pikud HaOref |
| alerts.com.ua | Ukrainian oblast air-raid alerts (EN names) | alerts.com.ua |
| Neptun | Ukraine real-time drone/missile/KAB tracks | neptun.in.ua |
| Google News RSS | Keyword/location-scoped news aggregation | Google |
| Google Translate | Hebrew/Arabic/Farsi + Ukrainian/Russian translation | Google *(unofficial)* |
| Telegram | Public channel posts via embed scraping | Telegram |
...
| Feed | Interval |
|------|---------:|
| 🚨 Air-raid alerts (Pikud HaOref / alerts.com.ua) | 15 sec |
| 🚁 Drone / missile tracker (Neptun) | 20 sec |
| 📨 Telegram channels | 60 sec |
| 🌡 Regional threat monitor | 60 sec |
| 📰 News RSS | 90 sec |
| 💥 Strikes | 2 min |
| ✈️ Conflicts / Flights | 3 min |
| 🚢 Naval | 5 min |
| 💹 Markets, Oil, Crypto & Polymarket | 10 min |
| 🔥 Fires (NASA FIRMS) | 10 min |
...
```text
src/
├── app/               # Next.js App Router + one API route per feed
│   └── api/           # news, telegram, alerts, drones, flights, ships,
│                      # strikes, conflicts, fires, markets, oil, crypto, ...
├── components/
│   ├── panels/        # 16 dashboard panels (NewsFeed, AlertsPanel, ...)
│   └── map/           # ConflictMap — Leaflet, all theater layers
├── lib/
│   ├── conflicts/     # ⭐ per-theater configs (iran-israel, russia-ukraine)
│   ├── hooks.ts       # useDataFeed / useConflictFeed polling hooks
│   ├── fetcher.ts     # timeout-guarded fetch + RSS/XML parsing
│   └── hebrew.ts      # Hebrew/Arabic/Farsi + Cyrillic translation layer
...
If you are a provider listed here and would like your source removed or its attribution changed, please [open an issue](https://github.com/DesusLove/IronSight/issues/new).

<details>
<summary><strong>⚖️ Full Legal Disclaimer</strong></summary>
